...
| AI-провайдеры | VseGPT с автоматическим fallback между моделями |
...
5. Android sends AI requests to the backend; backend proxies them to VseGPT and keeps API keys server-side.
...
AI_API_KEY_FILE=backend/secrets/ai_api_key
AI_CHAT_URL=
AI_TEXT_MODEL=
AI_TITLE_MODEL=
AI_SUMMARY_MODEL=
...
В приложении есть два режима выбора AI:

- `plus` - основной режим через VseGPT.
- `auto` - автоматический выбор модели под текущий сценарий.
...
- Android calls the backend AI proxy instead of calling VseGPT directly.
