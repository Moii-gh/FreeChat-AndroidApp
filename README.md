...
5. AI-ответы приходят напрямую из Gemini или VseGPT в зависимости от режима и настроек.
...
AUTH_BASE_URL=http://10.0.2.2:4000/api/
```
...
- AI ключи сейчас читаются из root `.env` и попадают в `BuildConfig`.
- Для production-сценария лучше проксировать AI-запросы через собственный backend.
...
- Для локальной отладки Android-клиенту нужно явно указать корректный `AUTH_BASE_URL`.
